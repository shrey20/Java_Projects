...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    /**
     * Remove a vertex from the graph. Also removes all edges adjacent to the
     * vertex from the graph (all edges that have the vertex as a source or a
     * destination vertex).
     *
     * @param data the data item stored in the vertex to remove
     * @return true if a vertex with *data* has been removed, false if it was
     * not in the graph
     * @throws NullPointerException if data is null
     */
     ...
     * @throws IllegalArgumentException if either source or target or both are
     * not in the graph, or if its weight is < 0
     ...
     * @param source the data item contained in the source vertex for the edge
     * @param target the data item contained in the target vertex for the edge
     * @return true if the edge could be removed, false if it was not in the
     * graph
     * @throws IllegalArgumentException if either source or target or both are
     * not in the graph
     * @throws NullPointerException if either source or target or both are null
     ...
     * @throws IllegalArgumentException if either sourceVertex or targetVertex
    ...
    /**
     * Path objects store a discovered path of vertices and the overal distance
     * of cost of the weighted directed edges along this path. Path objects can
     * be copied using the copy constructor, and then extended to include new
     * edges and verticies, one at a time. In comparison to a predecessor table
     * which is sometimes used to implement Dijkstra's algorithm, this
     * eliminates the need for tracing paths backwards from the destination
     * vertex to the starting vertex at the end of the algorithm.
     */
    protected class Path implements Comparable<Path> {
        ...
        /**
         * Creates a new path containing a single vertex. Since this vertex is
         * both the start and end of the path, it's initial distance is zero.
         *
        ...
        /**
         * This copy constructor makes a copy of the path passed into it as an
         * argument. This copy can be extended and edited without effecting the
         * original
         *
        ...
        public Path(Path source) {
            ...
            this.dataSequence = new LinkedList<>();//make a new linked list
            int x = source.dataSequence.size();
            for (int i = 0; i < x; i++) { //traverse the passed list
                this.dataSequence.add(source.dataSequence.get(i));//copy the list
            }
            this.end = source.end;
        }
        ...
        public void extend(Edge edge) {
            this.distance = this.distance + edge.weight;//update weight
            this.dataSequence.add(edge.target.data);//update data sequence
            this.end = edge.target; //update end
        }

        /**
         * Allows the natural ordering of paths to be increasing with path
         * distance. When path distance is equal, the string comparison of end
         * vertex data is used to break ties.
         *
         * @param other is the other path that is being compared to this one
         * @return -1 when this path has a smaller distance than the other, +1
         * when this path has a larger distance that the other, and the
         * comparison of end vertex data in string form when these distances are
         * tied
         */
    ...
    /**
     * Uses Dijkstra's shortest path algorithm to find and return the shortest
     * path between two vertices in this graph: start and end. This path
     * contains an ordered list of the data within each node on this path, and
     * also the distance or cost of all edges that are a part of this path.
     *
     ...
     * algorithm
     * @throws NoSuchElementException when no path from start to end can be
     * found, including when no vertex containing start or end can be found
     */
    protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {
        try {
            PriorityQueue<Path> pq = new PriorityQueue<>(5000);//initialized priority queue to arbitarily large number
            Vertex begin = vertices.get(start);
            Vertex finish = vertices.get(end);

            if (begin == null || finish == null) { //if the passed start and end are not in the graph
                throw new NoSuchElementException(); // throw exception
            }

            Path visit = new Path(begin);
            List<T> explored = new LinkedList(); // List to store all explored vertices.
            explored.add(visit.end.data);
            for (Edge edge : begin.edgesLeaving) {
                Path ext = new Path(visit); // uses the overloaded constructor to modify the paths
                ext.extend(edge);
                pq.add(ext); //storing the paths in the priority queue
            }
            Path poll = pq.poll(); //poll makes use of the copmare funcition to remove shortest path.
            if (poll.end.data.equals(end)) {
                return poll;
            }
            for (int x = 1; x < (getVertexCount() + getEdgeCount()); x++) { 

                for (Edge edge : poll.end.edgesLeaving) { //traverses through the edges
                    if (explored.contains(edge.target.data) != true) {
                        Path ext = new Path(poll);
                        ext.extend(edge);
                        pq.add(ext);
                    }
                }
     ...
     * between vertex with data item start and vertex with data item end,
    ...
    /**
     * Returns the cost of the path (sum over edge weights) between start and
     * end. Uses Dijkstra's shortest path algorithm to find the shortest path.
     *
     * @param start the data item in the starting vertex for the path
